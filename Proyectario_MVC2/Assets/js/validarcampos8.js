...
function checkInputs(select){
    ...
    if(select == 'div1'){
        ...
        ///----Usuario-----
        if(usuarioAv == ''){
            setErrorFor(usuarioA, 'El usuario no puede estar en blanco');
        } else if(!isUsuario_LA(usuarioAv)){
            setErrorFor(usuarioA, 'El usuario no es válido');
        }else{
            setSuccessFor(usuarioA);
        }

        //-----Contraseña de Admin----
        if(passwordAv == ''){
            setErrorFor(passwordA, 'La contraseña no puede estar en blanco');
